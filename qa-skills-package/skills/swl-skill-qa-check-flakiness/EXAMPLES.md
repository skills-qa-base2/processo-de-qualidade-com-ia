...
## Pl
**Cenário:** QA pleno na "SaúdeJá" (telemedicina fictícia) tem testes de agendamento que parecem depender de ordem de execução, mas não há histórico de execução disponível pra confirmar.
**Input:** "Os testes de agendamento falham quando rodam em paralelo, mas só às vezes. Não tenho o histórico de execução do CI à mão."
...
## Sr
**Cenário:** Na "ClickIngressos" (venda de ingressos, alta demanda), dias antes de um lançamento, o tech lead pede pra só marcar a suíte inteira como "flaky, ignorar por enquanto" pra não travar o deploy, já que "sempre foi meio instável mesmo".
