...
Busque a especificação real: arquivo OpenAPI/Swagger, JSON Schema, ou contrato Pact já publicado no broker. Se não houver especificação formal, pergunte ao usuário onde o contrato está documentado — nunca infira campos, tipos ou obrigatoriedade a partir de uma única resposta de exemplo.
