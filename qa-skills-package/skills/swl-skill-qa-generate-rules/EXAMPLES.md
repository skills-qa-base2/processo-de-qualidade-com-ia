## Jr
**Cenário:** QA júnior na "Lojinha Verde" (e-commerce fictício de plantas, web) gera as regras de QA pela primeira vez, a partir de uma página de wiki já existente.
...
## Pl
**Cenário:** QA pleno na "AgroConecta" (marketplace B2B, integrações com ERPs de fornecedores) tem só parte do processo documentado.
**Input:** "Não temos wiki, mas o time sabe de cabeça algumas convenções. Vou te contar o que sei."
**Saída esperada:** A skill conduz a entrevista curta, documenta o que foi confirmado (ferramentas de automação, estrutura de pastas), e para os pontos sem certeza no time (política de dados de teste ao integrar com ERP de terceiro) deixa marcado como "a definir" em vez de assumir um padrão genérico de mercado.

## Sr
**Cenário:** QA sênior herda o sistema legado da "RotaCerta" (logística/entregas fictícia, integração com sistemas de rastreamento de terceiros) sem nenhuma documentação de QA, e o tech lead pede pra "gerar regras baseadas no que costuma ser padrão em projetos de logística", já que ninguém mais lembra as convenções antigas.
