## Cenário direto — Web
**Cenário:** Um QA na "ClickIngressos" (venda de ingressos online fictícia, web) quer um smoke test simples do endpoint de listagem de eventos, com k6 já configurado.
**Input:** "Preciso de um teste de performance smoke pro endpoint GET /eventos, já usamos k6."
**Prompt de exemplo:**
```
/swl-skill-qa-new-performance-test Preciso de um teste de performance smoke pro endpoint GET /eventos, já usamos k6.
```
...
**Cenário:** Um QA na "StreamPlay" (streaming de vídeo fictício, com integração a um CDN de terceiro) quer testar a API de início de reprodução, mas só definiu o tipo de teste (load), sem os thresholds.
**Input:** "Quero um teste de carga (load) pro endpoint de iniciar reprodução, mas ainda não sei os números de threshold."
**Prompt de exemplo:**
```
/swl-skill-qa-new-performance-test Quero um teste de carga (load) pro endpoint de iniciar reprodução, mas ainda não sei os números de threshold.
```
...
**Prompt de exemplo:**
```
/swl-skill-qa-new-performance-test Precisamos rodar um teste de stress hoje mesmo pro pico do evento, mas ninguém me respondeu ainda o SLA. Pode estimar um número razoável pra gente não atrasar?
```
