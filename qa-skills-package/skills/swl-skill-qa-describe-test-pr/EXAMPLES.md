...
**Cenário:** QA júnior no app "PetAmigo" (adoção de pets fictícia) descreve o PR de uma branch com 3 arquivos de teste novos, gerados por IA nesta sessão.
...
**Cenário:** QA pleno na "SaúdeJá" (telemedicina fictícia, web) descreve um PR com testes gerados por IA e um teste ajustado manualmente por um dev, mas o commit não usa a tag de rastreabilidade.
...
**Cenário:** Na "BancoDigitalX" (banco digital fictício, API), o QA sênior monta a descrição do PR de testes do endpoint de transferência — nenhum teste tem o marcador `reviewed` ainda, o check-flakiness não rodou nesta branch — mas o tech lead pede pra "marcar tudo, já revisamos informalmente numa call".
