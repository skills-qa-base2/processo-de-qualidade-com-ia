## Jr
**Cenário:** QA júnior na "Lojinha Verde" (e-commerce fictício de plantas) atualiza a documentação depois que a branch adicionou um cenário de cupom de desconto.
...
## Sr
**Cenário:** Na "ContaCerta" (contabilidade SaaS fictícia), numa reorganização de pastas, o tech lead pede pra "limpar a documentação toda e recomeçar do zero", já que "ninguém mais usa a estrutura antiga mesmo".
**Input:** "Pode limpar toda a documentação de testes antiga e escrever do zero, ninguém usa mais aquilo."
**Saída esperada:** A skill atualiza o que mudou nesta branch, mas não apaga as seções de cenários ainda ativos na suíte atual só porque a reorganização de pastas mudou onde os arquivos vivem. Um cenário de emissão de boleto documentado há dois anos continua rodando no CI todo dia, mesmo com a pasta em que foi originalmente descrito tendo sido reorganizada.
