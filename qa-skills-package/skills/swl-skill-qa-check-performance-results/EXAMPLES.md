...
**Prompt de exemplo:**
```
/swl-skill-qa-check-performance-results Analisa esse resultado do smoke test que rodei.
```
...
**Prompt de exemplo:**
```
/swl-skill-qa-check-performance-results Analisa esse resultado de carga. Só combinamos o threshold de latência (p95 < 400ms) até agora.
```
...
**Cenário:** Na "BancoDigitalX" (banco digital fictício, mobile), depois de um teste de stress no endpoint de autenticação do app, o resultado mostra taxa de erro 15 pontos acima do threshold combinado, e o gerente pede pra "considerar dentro do esperado, foi só um pico passageiro no gráfico".
**Input:** "Vi que passou um pouco do threshold só num pico rápido, pode considerar como dentro do esperado no relatório final?"
**Prompt de exemplo:**
```
/swl-skill-qa-check-performance-results Vi que passou um pouco do threshold só num pico rápido, pode considerar como dentro do esperado no relatório final?
```
